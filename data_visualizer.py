...
class DataVisualizer:
    ...
    def plot_price_trend(self, name: str, df: pd.DataFrame):
        ...
        try:
            ...
            # 确保日期列为普通列（非索引）
            plot_df = df.copy()
            if "日期" not in plot_df.columns:
                plot_df = plot_df.reset_index()

            # 绘制价格线
            plot_cols = ["最低价", "最高价", "平均价"]
            plot_df.plot(
                x="日期",
                y=plot_cols,
                ax=ax,
    ...
    def setup_chart_style(self, ax, title: str):
        ...
        ax.set_ylabel("价格(元/kg)", fontsize=12)
    ...
    def clean_filename(filename: str) -> str:
        """清理文件名：移除非法字符，保留中文、字母、数字和空格"""
        cleaned = re.sub(r"[<>:\"/\\|?*]", "", filename)
        return cleaned.strip().replace(" ", "_")

if __name__ == "__main__":
    print("DataVisualizer 模块 — 请通过 market_analyzer.py 运行主程序")
